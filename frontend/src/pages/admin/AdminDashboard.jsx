...
// --- UPDATED TOP BAR WITH MANAGE ACCOUNT ---
    ...
    <div className={`fixed top-0 right-0 z-40 flex items-center justify-between px-8 py-4 bg-[#f4f2f6]/80 backdrop-blur-md transition-all duration-300 ${sidebarOpen ? 'left-72' : 'left-20'}`}>
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative">
          <button onClick={toggleNotifs} className={`p-2.5 rounded-xl transition-all shadow-sm border border-gray-100 relative ${showNotifs ? 'bg-[#afa2ba] text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>
            ...
            <>
              ...
              <div className="absolute right-0 top-14 w-56 bg-white rounded-xl shadow-2xl border border-gray-100 z-50 animate-slide-up overflow-hidden">
                <div className="p-2 border-b border-gray-50 space-y-1">
                  ...
                  {/* ADDED MANAGE ACCOUNT */}
                  <button onClick={() => handleNavigation('/settings')} className="w-full text-left px-3 py-2 text-sm font-bold text-gray-700 hover:bg-purple-50 rounded-lg flex items-center gap-2">
...
// ... REST OF THE ADMIN DASHBOARD CODE (AddPropertyModal, etc.) REMAINS THE SAME ...
// Paste the rest of your AddPropertyModal, NotificationDropdown, etc. here or keep them as they were.
...
const AddPropertyModal = ({ onClose, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 font-sans">
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl relative z-10 overflow-hidden animate-slide-up border border-white/20 flex flex-col max-h-[90vh]">
        <div className="bg-[#afa2ba] px-8 py-6 flex justify-between items-center flex-shrink-0">
        ...
        <div className="p-8 overflow-y-auto custom-scrollbar space-y-8 flex-1">
           <div className="space-y-4">
             <h4 className="font-bold text-gray-800 border-b pb-2">Basic Information</h4>
             <div className="grid grid-cols-2 gap-6">
                <div className="col-span-2"><label className="block text-sm font-bold text-gray-700 mb-2">Property Name</label><input name="name" onChange={handleChange} type="text" placeholder="e.g. SMDC Grass Residences Tower 4" className="w-full border border-gray-300 rounded-xl py-3 px-4 outline-none focus:border-[#afa2ba]" /></div>
                <div className="col-span-2"><label className="block text-sm font-bold text-gray-700 mb-2">Address</label><input name="address" onChange={handleChange} type="text" placeholder="Full Address" className="w-full border border-gray-300 rounded-xl py-3 px-4 outline-none focus:border-[#afa2ba]" /></div>
                <div className="col-span-2"><label className="block text-sm font-bold text-gray-700 mb-2">Description</label><textarea name="description" onChange={handleChange} placeholder="Describe the property features, amenities, etc." rows={3} className="w-full border border-gray-300 rounded-xl py-3 px-4 outline-none focus:border-[#afa2ba]" /></div>
                <div><label className="block text-sm font-bold text-gray-700 mb-2">Bedrooms</label><input name="bedrooms" onChange={handleChange} type="number" className="w-full border border-gray-300 rounded-xl py-3 px-4 outline-none focus:border-[#afa2ba]" /></div>
                <div><label className="block text-sm font-bold text-gray-700 mb-2">Bathrooms</label><input name="bathrooms" onChange={handleChange} type="number" className="w-full border border-gray-300 rounded-xl py-3 px-4 outline-none focus:border-[#afa2ba]" /></div>
                <div><label className="block text-sm font-bold text-gray-700 mb-2">Price (₱ / Month)</label><input name="price_per_month" onChange={handleChange} type="number" placeholder="0.00" className="w-full border border-gray-300 rounded-xl py-3 px-4 outline-none focus:border-[#afa2ba]" /></div>
                <div><label className="block text-sm font-bold text-gray-700 mb-2">Size (sqm)</label><input name="size_sqm" onChange={handleChange} type="number" placeholder="0" className="w-full border border-gray-300 rounded-xl py-3 px-4 outline-none focus:border-[#afa2ba]" /></div>
             </div>
           </div>
           <div className="space-y-4">
             <div className="flex justify-between items-center border-b pb-2">
               <h4 className="font-bold text-gray-800">Property Images <span className="text-sm font-normal text-gray-500">(Min 3, Max 10)</span></h4>
             ...
             <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:bg-gray-100 transition-colors cursor-pointer relative">
               <input type="file" multiple accept="image/*" onChange={handleFileChange} className="absolute inset-0 opacity-0 cursor-pointer" />
               <div className="flex flex-col items-center">
                 <Icon name="Image" size={32} className="text-gray-400 mb-2" />
                 <p className="text-sm font-bold text-gray-600">{uploading ? "Uploading..." : "Click or Drag to Upload Images"}</p>
             ...
             {formData.images.length > 0 && (
               <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4 mt-4">
                 {formData.images.map((img, idx) => (
                   <div key={idx} className="relative aspect-square group rounded-xl overflow-hidden border border-gray-200 shadow-sm bg-gray-100">
        ...
        <div className="p-6 bg-gray-50 border-t border-gray-200 flex justify-end gap-4 flex-shrink-0">
          <button onClick={onClose} className="px-8 py-3 text-gray-500 font-bold hover:bg-gray-200 rounded-xl text-sm">Cancel</button>
          <button disabled={uploading} onClick={handleSubmit} className={`px-10 py-3 text-white font-bold rounded-xl shadow-lg text-sm ${uploading ? 'bg-gray-400' : 'bg-[#afa2ba] hover:bg-[#9a8a9b]'}`}>
...
const NotificationDropdown = ({ notifications, isOpen, onClose }) => {
  ...
  return (
    <div className="absolute top-20 right-8 w-96 bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 animate-slide-up overflow-hidden">
      ...
      <div className="max-h-[400px] overflow-y-auto custom-scrollbar">
        {notifications.length === 0 ? <div className="p-8 text-center text-gray-400 text-sm">No new notifications</div> : notifications.map((notif, index) => (
          <div key={index} className="px-6 py-4 border-b border-gray-50 hover:bg-gray-50 transition-colors cursor-pointer flex gap-4 relative group">
            <div className={`h-10 w-10 rounded-full flex items-center justify-center flex-shrink-0 ${notif.type === 'payment' ? 'bg-green-100 text-green-600' : 'bg-purple-100 text-purple-600'}`}>
      ...
      <div className="p-3 border-t border-gray-100 bg-gray-50 text-center"><button onClick={onClose} className="text-xs font-bold text-gray-500 hover:text-gray-800">Close</button></div>
    </div>
  );
};
      ...
      <div className="relative bg-white p-2 rounded-xl max-w-2xl w-full shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="rounded-lg overflow-hidden bg-gray-100 flex items-center justify-center min-h-[400px]">
           <div className="text-center w-full">
             ...
             <code className="bg-gray-200 px-2 py-1 rounded text-xs text-gray-600 block mt-2 mx-auto w-fit">{src}</code>
           </div>
        </div>
      </div>
    </div>
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 font-sans">
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md relative z-10 overflow-hidden animate-slide-up">
        ...
        <div className="p-6 bg-gray-50 border-t border-gray-200 flex justify-end gap-4">
          ...
          <button onClick={()=>onSave(formData)} className="px-6 py-2 bg-[#afa2ba] text-white font-bold rounded-xl shadow-lg hover:bg-[#9a8a9b] text-sm">Save</button>
        </div>
      </div>
    </div>
  );
...
const AdminDashboard = () => {
  ...
  const StatusPill = ({ status }) => {
    ...
    let color = (s === 'confirmed' || s === 'active' || s === 'approved' || s === 'completed' || s === 'paid') ? 'text-green-600 bg-green-50' : (s === 'pending' || s === 'occupied') ? 'text-yellow-600 bg-yellow-50' : 'text-red-600 bg-red-50';
    let label = status;
    if (status === true) { label = 'Available'; color = 'text-green-600 bg-green-50'; }
    if (status === false) { label = 'Booked'; color = 'text-red-600 bg-red-50'; }
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-[#dcd3e2] text-purple-800 font-bold">Loading Dashboard...</div>;
      ...
      <div className={`transition-all duration-300 min-h-screen flex flex-col ${sidebarOpen ? 'ml-72' : 'ml-20'}`}>
        ...
        <div className="p-10 mt-24 flex-1">
             ...
             <>
                <div className="grid grid-cols-3 gap-6 mb-8">
                    {[{l:'Revenue', v: `₱${(stats?.total_revenue || 0).toLocaleString()}`}, {l:'Users', v: stats?.total_users || 0}, {l:'Bookings', v: stats?.total_bookings || 0}].map((s,i) => (
                        <div key={i} className="bg-white rounded-2xl p-6 shadow-sm flex justify-between items-center h-32">
                            <span className="text-4xl font-bold text-purple-400 opacity-70">{s.v}</span>
                            <span className="text-lg font-bold text-gray-700">{s.l}</span>
                        </div>
                    ))}
                </div>
                <div className="mb-8">
                    <h3 className="text-xl font-bold mb-4 text-gray-800">Pending Approvals</h3>
                    <DataTable 
                        data={condoData.filter(p => p.status === 'pending')}
                        columns={[
                            { header: 'Name', accessor: 'name', className: 'p-4 font-bold' },
                            { header: 'Price', render: (r) => `₱${r.price_per_month?.toLocaleString()}`, className: 'p-4' },
          ...
          {activePage === 'properties' && <TableView title="All Properties" addButtonText="Add Property" onAdd={() => setShowAddPropertyModal(true)} data={condoData} columns={[
            { header: 'Name', accessor: 'name', className: 'p-3 text-sm font-bold' },
            { header: 'Loc', accessor: 'address', className: 'p-3 text-sm text-gray-500' },
            { header: 'Price', render: (r) => `₱${r.price_per_month?.toLocaleString()}`, className: 'p-3 text-sm font-bold text-green-600' },
            { header: 'Status', render: (r) => <StatusPill status={r.status} />, className: 'p-3 text-center' },
            { header: 'Actions', render: (row) => row.status === 'pending' ? <span className="text-xs text-yellow-600 font-bold">Action Needed</span> : <span className="text-xs text-gray-400">Managed</span>, className: 'p-3 text-center' }
          ]} />}

          {activePage === 'owners' && <TableView title="Owners" data={ownersData} columns={[{ header: 'Name', accessor: 'name', className: 'p-4 font-bold' },{ header: 'Email', accessor: 'email', className: 'p-4 text-blue-600' },{ header: 'Actions', render: (row) => <UserActionButtons row={row} type="owners" />, className: 'p-4 text-center' }]} />}
          {activePage === 'tenants' && <TableView title="Tenants" data={tenantsData} columns={[{ header: 'Name', accessor: 'name', className: 'p-4 font-bold' },{ header: 'Email', accessor: 'email', className: 'p-4 text-blue-600' },{ header: 'Actions', render: (row) => <UserActionButtons row={row} type="tenants" />, className: 'p-4 text-center' }]} />}
          {activePage === 'payments' && <TableView title="Payments" data={paymentsData} columns={[
            { header: 'User', accessor: 'user', className: 'p-3 text-xs' },{ header: 'Amount', accessor: 'amount', className: 'p-3 text-xs font-bold' },{ header: 'Status', render: (r)=><StatusPill status={r.status}/>, className: 'p-3 text-center' },
