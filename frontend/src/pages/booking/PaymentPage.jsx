...
// --- EXTRACTED COMPONENTS (Defined OUTSIDE PaymentPage) ---
...
const MethodSelectionView = ({ paymentMethod, setPaymentMethod, setStep, agreedTerms, setAgreedTerms }) => (
  <div className="max-w-2xl mx-auto mt-8 bg-white p-8 rounded-3xl shadow-xl border border-gray-100 animate-fade-in">
      ...
      <button 
        ...
        onClick={() => setStep(3)} 
        className={`w-full py-4 rounded-xl text-white font-bold text-xl tracking-wider shadow-md transition-all ${!paymentMethod ? 'bg-purple-300 cursor-not-allowed' : 'bg-[#4b0082] hover:bg-purple-600'}`}
      >
        NEXT
      </button>
  </div>
...
const PaymentDetailsView = ({ paymentMethod, handleFinalPaymentSubmission, loading, agreedTerms, setAgreedTerms }) => (
  <div className="max-w-3xl mx-auto mt-8 bg-white p-8 rounded-3xl shadow-xl border border-gray-100 animate-fade-in">
    ...
    <div className="space-y-6 mb-10 px-4">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
              <div>
                  ...
                  <input type="text" className="w-full border-2 border-gray-300 rounded-lg p-3" placeholder="MM/YY" />
              </div>
              <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">CVC</label>
                  <input type="text" className="w-full border-2 border-gray-300 rounded-lg p-3" placeholder="123" />
              </div>
          </div>
        </div>
      )}
      {paymentMethod === 'GCASH' && (
        <div>
            <label className="block text-sm font-bold text-gray-700 mb-1">GCash Number</label>
            <input type="text" className="w-full border-2 border-gray-300 rounded-lg p-3" placeholder="09XX XXX XXXX" />
        </div>
      )}
      {paymentMethod === 'CASH' && (
           <div className="bg-yellow-50/50 p-6 rounded-xl text-yellow-800 font-bold text-center border border-yellow-300">
              Cash Payment selected. Please proceed to submit your booking. You will pay at the Admin office.
          </div>
      )}
    </div>
    ...
    <button 
      ...
      className={`w-full text-white font-bold text-xl py-4 rounded-xl shadow-md transition-all uppercase tracking-widest flex justify-center items-center gap-2 ${loading || !agreedTerms ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#4b0082] hover:bg-purple-600'}`}
    >
...
const ConfirmationView = ({ isSuccess, setStep, navigate }) => (
      ...
      <div className="flex justify-center gap-6">
        {isSuccess ? (
            <button 
              ...
              className="bg-[#4b0082] text-white font-bold text-lg px-8 py-3 rounded-lg hover:bg-purple-600 transition-colors uppercase shadow-md"
            >
        ...
        <button 
          onClick={() => navigate('/')} 
          className="bg-white border-2 border-[#4b0082] text-[#4b0082] font-bold text-lg px-8 py-3 rounded-lg hover:bg-purple-50 transition-colors uppercase shadow-md"
        >
          BACK TO HOME
        </button>
      </div>
...
const PaymentPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
        try {
            // Assuming api.get returns the response object, access .data if needed or configured in interceptor
            const response = await api.get(`/bookings/${bookingId}`);
            // Use response directly if interceptor unwraps it, or response.data if not
  ...
  // --- Handlers ---
  const handleIdUpload = useCallback((event) => {
  ...
  const handleFinalPaymentSubmission = async () => {
    ...
    try {
        ...
        if (!intentRes.payment_intent_id) {
      ...
      <div className="px-4 pt-10">
        ...
        <div className="animate-fade-in">
            ...
            <div className="bg-white p-8 rounded-3xl shadow-xl border border-gray-100 animate-fade-in">
              ...
              <div className="bg-blue-50 border-l-4 border-blue-400 p-6 rounded-r-lg mb-8">
                <h3 className="text-lg font-bold text-blue-800 mb-4 flex items-center gap-2">
                  <User size={20} className="text-blue-600" /> Logged-in Profile Details (Read-only)
                </h3>
          ...
          {step === 3 && (
            <PaymentDetailsView 
              paymentMethod={paymentMethod} 
              handleFinalPaymentSubmission={handleFinalPaymentSubmission} 
              loading={loading}
              agreedTerms={agreedTerms} 
              setAgreedTerms={setAgreedTerms} 
            />
